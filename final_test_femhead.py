...
def main():
    ...
    Dataset = getattr(datasets,"HipSegDatasetTESTFEM")
    ...
    # Define test dataset
    test_data = Dataset(data_dir,params.image_dir,params.target_dir,target_sfx=params.target_sfx,
                           input_tf=transform,output_tf=target_transform)

    if args.roi:
        pred_test_data = Dataset(data_dir,params_roi.image_dir,params_roi.target_dir,target_sfx=params.target_sfx,
                                 input_tf=transform,output_tf=target_transform)
    ...
    if args.roi:
        pred_acc = test(model,device,pred_test_loader,metrics,params_roi,checkpoint=chkpt,name=args.model_name,extra="_Pred"+extra2, 
               prediction_dir=prediction_roi)
